...
vector<queue<Data*>> bucketOfBuckets(10000);

//literally every single first/last name for radix sorting. its preordered so its easier. 
...
void radixSort(list<Data*>* l){
  ...
  for (it = l -> begin(); it != l -> end(); ++it){
    for (radixing = 0; radixing < firstNamesSorted.size(); radixing++){
      if ((*it) -> firstName == firstNamesSorted[radixing]){
        break;
      }
    }//when we break out of the loop we will have the index to stick the name in
    bucketOfBuckets[radixing].push(*it);
  ...
  for (it = l -> begin(); it != l -> end(); ++it){
    for (radixing = 0; radixing < lastNamesSorted.size(); radixing++){
      if ((*it) -> lastName == (lastNamesSorted[radixing])){
        break;
      }
    }//when we break out of the loop we will have the index to stick the name in
    bucketOfBuckets[radixing].push(*it);
